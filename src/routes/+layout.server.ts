...
export const load: LayoutServerLoad = async () => {
	...
	try {
		...
		const TEN_HOURS_IN_MS = 10 * 3600 * 1000;
		const ONE_DAY_IN_MS = 24 * 3600 * 1000;

		const currentTicketDayNumber = Math.floor((now.getTime() - TEN_HOURS_IN_MS) / ONE_DAY_IN_MS);
		const lastTicketDayNumber = lastTicketDate
			? Math.floor((lastTicketDate.getTime() - TEN_HOURS_IN_MS) / ONE_DAY_IN_MS)
			: -1;

		if (now.getUTCHours() >= 10 && currentTicketDayNumber > lastTicketDayNumber) {
			await addTicket();
